...
require_relative 'app/models_pg'
...
delete '/api/recipes/:id/save' do
  ...
  Models::UserAction.exec(
    "DELETE FROM user_actions WHERE user_id = $1 AND recipe_id = $2 AND action_type = 'save'",
    [current_user.id, params[:id]]
  )

  # save_countを減らす
  Models::Recipe.exec(
    "UPDATE recipes SET save_count = CASE WHEN save_count > 0 THEN save_count - 1 ELSE 0 END WHERE id = $1",
    [params[:id]]
  )
...
if __FILE__ == $0
  ...
  if Models::Recipe.all.empty?
    puts "📦 PostgreSQLシードデータを投入中..."
    Database.seed!
  end

  port = settings.port
  puts "\n" + "="*60
  puts "🚀 ミーネクストサーバーが起動しました (PostgreSQL版)"
  puts "="*60
  ...
  puts "💾 データベース:    PostgreSQL (minext_development)"
  puts "="*60
  puts "\n✨ 開発を楽しんでください！\n\n"
end
